...
/**
 * Validate that a report ID is safe to use in file paths.
 * Report IDs are UUIDs generated by randomUUID(). Reject anything that could
 * traverse the filesystem (e.g. "..", "/", "\", null bytes).
 */
function isValidReportId(id: string): boolean {
  // UUID v4 format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (36 chars)
  // Also allow alphanumeric IDs for backwards compat.
...
export function compareReports(
  ...
  if (!isValidReportId(oldId) || !isValidReportId(newId)) {
    return { idea: "", deltas: {}, overallTrend: "unknown" };
  }
...
function readPartialJson(filePath: string, maxBytes: number): Partial<GauntletReport> | null {
  try {
    ...
    // Try full JSON.parse on the partial buffer first.
    // If the file is smaller than maxBytes, this succeeds and gives us all fields
    // including ones with escaped quotes or newlines that regex would miss.
    try {
      const parsed = JSON.parse(text) as Partial<GauntletReport>;
      if (parsed?.id && parsed?.createdAt) return parsed;
    } catch {
      // Partial buffer isn't valid JSON — fall through to regex extraction.
    }

    // Regex fallback: extract top-level fields by looking for well-known keys.
    // This handles cases where the buffer is truncated mid-object.
